...
// ── Rotation parent: mat3 applied ──
{
  ...
  // A=0 → identity rotation → (10, 0) + origin (100, 100) = (110, 100)
  let frame = evalRig(rigSpec, { A: 0 })[0];
  assert(arrEq(frame.vertexPositions, [110, 100]), 'rotation 0°: pivot-relative + origin');
  // A=90 → rotate (10, 0) → (0, 10) + origin = (100, 110)
  frame = evalRig(rigSpec, { A: 90 })[0];
  assert(arrEq(frame.vertexPositions, [100, 110], 1e-5), 'rotation 90°: 90° around origin');
  // A=-90 → rotate (10, 0) → (0, -10) + origin = (100, 90)
  frame = evalRig(rigSpec, { A: -90 })[0];
  assert(arrEq(frame.vertexPositions, [100, 90], 1e-5), 'rotation -90°: -90° around origin');
}
