...
/** @deprecated since Phase 2 audit fix — use `pickSelectionAnchor`.
 *  Old shape: 'closest' meant "the cursor IS the anchor", which is
 *  not Blender's semantics (Blender's closest = nearest selection
 *  vertex to the snap target). Kept for one release so legacy tests
 *  don't break; new callers should not use it.
 *
 *  Maps the legacy contract:
 *    - 'closest' → returns opts.cursor verbatim
 *    - 'center'  → AABB centre of verts (or cursor when empty)
 *    - 'median'  → per-axis median (or cursor when empty)
 *    - 'active'  → opts.activeVert (or cursor when missing)
 */
export function computeSelectionAnchor(verts, target, opts) {
  ...
  if (target === 'closest' || !target) return cursor;
