...
> - [../../WORKSPACES.md](../../WORKSPACES.md) — live workspace / editMode / toolbar / click-to-select contract
> - [BUGS.md](../../BUGS.md) / [FEATURE_GAPS.md](../../FEATURE_GAPS.md) — running task lists
> - [WORKSPACE_MODE_REWORK.md](WORKSPACE_MODE_REWORK.md) — 5→3 workspace collapse + single-slot `editorStore.editMode` decision record
> - [CLICK_TO_SELECT.md](CLICK_TO_SELECT.md) / [TOOLBAR.md](TOOLBAR.md) — both SHIPPED 2026-05-02
...
| 1F.6 Live Preview / Edit-mode separation | ✅ shipped | `d875f72` | New `livePreviewActive` flag in editorStore. Edit mode (default): physics tick + breath + cursor look gated off; sliders are the only writers to paramValuesStore — they don't dance during editing. Live Preview mode: physics runs, ParamBreath auto-cycles at Cubism's ~3.345s standard, LMB-drag drives ParamAngleX/Y/Z (±30°). Toggle button + status text in ParametersEditor header. Snapshot/restore around the session preserves slider values. **(Superseded twice on 2026-05-02. First by the initial GAP-010 shape — `livePreviewActive` removed, two distinct editor types `viewport`/`livePreview` registered, drivers bound to `<LivePreviewEditor>` mount lifetime. Then refactored same day into the final shape: a single shared [`<CanvasArea>`](../../../src/v3/shell/CanvasArea.jsx) host that both canvas tabs back onto, with `previewMode` flipping on a stable CanvasViewport instance. The intermediate two-component shape blew away the WebGL context on every tab toggle and lost the wizard's local PSD state — fix is BUG-013. See [docs/FEATURE_GAPS.md](../../FEATURE_GAPS.md#-gap-010--live-preview-as-a-tab-on-the-center-area).)** |
