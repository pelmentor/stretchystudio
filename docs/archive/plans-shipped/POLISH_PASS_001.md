...
**Type:** bug · **Severity:** high · **Status:** closed (commits `1c633ec` + audit follow-up)

**Root cause.** The previously-shipped GAP-008 work (commit `41e63bc`) added the subsystem-opt-out filter on the **authored-cmo3** path and on the **harvested seed-output** for the heuristic path (`project.rigWarps` storage, `project.physicsRules` storage). What it didn't filter was the live **`rigSpec.warpDeformers`** that comes back from `generateCmo3` on the heuristic path — chainEval consumes that spec directly, so hair-tagged rigWarps that were dropped from the seed-output were still getting evaluated every frame, producing the visible "hair sways during body lean" symptom.

Fix: added `applySubsystemOptOutToRigSpec(rigSpec, {subsystems, nodes})` in [`initRig.js`](../../../src/io/live2d/rig/initRig.js). The first cut at this fix tried to mirror `buildRigSpecFromCmo3`'s drop+reparent strategy, but that's wrong for the heuristic path: art mesh verts on the heuristic path are pre-projected into the dropped warp's normalised 0..1 frame, and reparenting without re-projection would put hair parts at wildly wrong locations. The audit-pass rewrite **neutralises** the warps instead — bindings cleared, keyforms reduced to the single rest one. `cellSelect` with empty bindings returns the rest keyform with weight 1, so chainEval evaluates the warp as a coord-correct pass-through and the hair stays at its rest pose with no sway.
...
- Plan governing this audit's parent feature: [V3_RERIG_FLOW.md](V3_RERIG_FLOW.md) (shipped 2026-05-03)
