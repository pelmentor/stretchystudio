...
Not in scope: V4 Track 3 (Keyform Editor) and Track 4 (Weight Paint v2) — they have their own plan in [docs/V4_BLENDER_PARITY.md](V4_BLENDER_PARITY.md) and depend on this work landing first.
...
| 3 | Phase 3 — `armatureEdit` rotates around what? | TBD; default to "no-op for now, add `rest.rotation` later" | 2D-rest limitation already documented in [REST_POSE_SPLIT.md](REST_POSE_SPLIT.md); deferring full rest rotation is consistent |
...
- **Keyform Editor UI** (V4 Track 3) — has its own [V4_BLENDER_PARITY.md](V4_BLENDER_PARITY.md) sequence; depends on Phase 1 visual landing first.
- **Weight Paint v2** (V4 Track 4) — same.
- **Param editor edit operations** (V4 Track 2) — same.
- **`rest.rotation` for bones** — full Blender parity (3D-style rest matrix) is deferred per [REST_POSE_SPLIT.md](REST_POSE_SPLIT.md). Phase 3 here works around this by disabling R/S in armatureEdit.
...
- [REST_POSE_SPLIT.md](REST_POSE_SPLIT.md) — schema v17 data layer this plan builds on
- [V4_BLENDER_PARITY.md](V4_BLENDER_PARITY.md) — sibling plan (Tracks 2/3/4)
- [BFA_006_DEFORMER_NODES.md](BFA_006_DEFORMER_NODES.md) — deformer-as-node refactor that closed sidetables
