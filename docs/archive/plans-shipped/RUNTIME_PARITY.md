...
>
> Current parity status: model loads, deforms (warp + rotation + variant fade + 2D eye compound + clip masks), blinks, tracks cursor, no warnings. Memory note: `project_runtime_export_parity.md`. Open numerical residuals (Cubism warp port Phase 2b / BUG-003 AngleZ peak) are tracked in [CUBISM_WARP_PORT.md](../../live2d/CUBISM_WARP_PORT.md).
...
semantics](../../live2d/MOC3_FORMAT.md#compile-time-semantics-cmo3--moc3) for details.
...
After the [native rig refactor](NATIVE_RIG_REFACTOR.md) landed (Stages
