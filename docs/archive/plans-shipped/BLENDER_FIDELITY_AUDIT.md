...
**Plan.** Promote deformers to `project.nodes` entries with `type:'deformer'`. `rigSpec` becomes a derived selector — a runtime index over `project.nodes`, not a separately-built blob. Three persistent sidetables collapse into the node list itself. 7-phase migration; full plan in [BFA_006_DEFORMER_NODES.md](BFA_006_DEFORMER_NODES.md).
