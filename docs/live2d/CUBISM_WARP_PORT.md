...
v3's evaluator was written from scratch during the [Native Rig Refactor](../archive/plans-shipped/NATIVE_RIG_REFACTOR.md)
...
**Status:** 🟠 Planned. Two single-sweep attempts (option b: J⁻¹ swap; option a: canvas-final break) both reverted on 2026-05-02. Refactor plan with explicit stages + oracle gates between stages now lives at [archive/plans-shipped/PHASE_2B.md](../archive/plans-shipped/PHASE_2B.md) (authored 2026-05-03, 3.5-day estimated cost; SHIPPED 2026-05-03).
...
- **2026-05-02** — Oracle harness shipped via `cmo3Import` path. [`scripts/cubism_oracle/diff_v3_vs_oracle.mjs`](../../scripts/cubism_oracle/diff_v3_vs_oracle.mjs) reads `shelby.cmo3`, calls `importCmo3` + `initializeRigFromProject` in Node (with browser-API stubs for `Blob` + `URL.createObjectURL`), runs `evalRig` against each pinned oracle snapshot, computes per-drawable max + mean canvas-px diff. Splits TOTAL divergence (raw v3 vs oracle) from PARAM-DRIVEN divergence (total minus rest baseline) so static rigSpec-build accuracy doesn't pollute the BUG-003 signal. Now the canonical pass criterion as planned in 2026-05-01's known-gap entry — no longer "scaffolded conceptually". 21 fixtures × per-drawable diff in <2s.
- **2026-05-02** — Phase 2a kernel verification via raw asm. Earlier "Phase 2a port" (commit `0f512d0`) was reverted same day (mathematically wrong: produced x↔y swap at θ=0). On a re-RE pass with raw asm at IDA `0x7fff2b24c950` (NOT Hex-Rays pseudocode) plus careful `xmm5/xmm6/xmm7/xmm8` register tracking, the actual kernel reads as `out.x = px·(cos·s·rX) + py·(-sin·s·rY) + originX; out.y = px·(sin·s·rX) + py·(cos·s·rY) + originY` — exactly textbook 2D rotation with `diag(rX, rY)` post-scale. Identical to v3's existing `buildRotationMat3Aniso`. **No port file needed.** `cubismRotationEval.js` retained as historical record only. The earlier "register tracking confused which mulss target paired with which mulss source" turned out to be an `xmm5 = -sin·s·rY` (multiplied with `py` after `xmm1 *= xmm5`) vs `xmm6 = sin·s·rX` (multiplied with `px` after `xmm2 *= xmm6`) confusion.
- **2026-05-02** — Phase 2b implementation BLOCKED by architectural mismatch. Two candidate implementations both made oracle divergence WORSE than baseline:
  - **Canvas-final + chain-stop** (mirror Cubism's "Setup absorbs the chain into per-deformer state, eval is canvas-final per call"): 1700× worse (max=85,000 px). Root cause: v3's chain walker doesn't feed `canvas-px relative to pivot` to rotation deformers — it feeds whatever the previous step output (typically a warp's normalised 0..1). Cubism's Setup makes child frames consistent canvas-px; v3 doesn't.
  - **FD-magnitude-as-slope** (replace `_warpSlopeX/Y` with `1 / probedScale` from Y-direction probe): 2× worse (param max=36 px vs 17 px baseline). Root cause: v3's rotation matrix is `R · diag(extraSx, extraSy)` (diagonal scaling only). FD probe at parameter-rotated warps gives a 2D delta vector with off-diagonal directional info; v3's diagonal matrix can only carry the magnitude `|delta|`, not the rotation. The closed-form `_warpSlopeX/Y` happens to be a better approximation precisely BECAUSE it doesn't try to capture rotation it can't represent anyway.
  Real Phase 2b fix requires switching `rotationEval.js`'s `buildRotationMat3Aniso` + downstream consumers to a general 2x2 + translation matrix (full Jacobian). That's a downstream refactor out of scope for a single sweep. `DeformerStateCache.evalChainAtPoint` is preserved as infrastructure for the next attempt.
- **2026-05-02** — Decision log of harness limitations: Node has no `Image` decoder, so eye-closure parabola fits fall back to mesh-bin-max in the harness. Eye meshes (eyelash, eyewhite, irides) show ~13-66 px static rest divergence vs oracle BECAUSE of this; eyebrows + hair (no eye-closure handling) show ~0.07 px rest divergence — confirming Phase 1 warp eval is correct at rest. PARAM-DRIVEN divergence (computed by subtracting rest baseline) is unaffected and remains the BUG-003 signal. Eventual fix: integrate a Node PNG decoder (sharp / `png-js` / similar) so the harness sees real alpha; not blocking the BUG-003 quantification.
- **2026-05-02** — Breath warp byte-fidelity verification: `scripts/dev-tools/moc3_inspect_warp.py` against `New Folder_cubism/shelby.moc3` confirms our heuristic `BreathWarp` grid bytes match Cubism Editor's authored shelby breath warp **byte-for-byte** at both kf[0] (rest) and kf[1] (full breath). Same for the deformer chain topology (BZ→BY→BR→BX→rotations). Pinned by [`test:breathFidelity`](../../scripts/test/test_breathFidelity.mjs) (66 cases). Investigation triggered by user "breath doesn't match Cubism Viewer" — the residual visual divergence was the chain-composition issue addressed by Phase 3 next.
- **2026-05-02** — Live preview cycle period correctness: ParamBreath synth in CanvasViewport used `cycle=3.345s`; Cubism Web Framework's `CubismBreath` standard wiring uses **`3.2345s`** for ParamBreath. The 0.11 s discrepancy made our breath drift relative to a Cubism Viewer playing the same model side-by-side. Fixed (commit `b6808a4`).
- **2026-05-02** — **Phase 3 SHIPPED + ORACLE-VERIFIED.** Lifted-grid Setup mirroring Cubism Core's `WarpDeformer_Setup` (IDA `0x7fff2b24e410`). Each warp's grid is composed top-down through ancestors to canvas-px once per frame via new `DeformerStateCache.getLiftedGrid()`; artmesh evaluation does ONE bilinear against the lifted grid instead of nested bilinears through the chain. Recursive + memoized + cycle-guarded (in-flight Set + try/finally; emits structured `warn` to Logs panel on cycle hit). The 2026-05-01 entry that said "Phase 3 verifies the equivalence rather than rewriting the orchestration" was WRONG — nested-bilinear composition produces a quartic when intermediate warps are non-identity, while Cubism's lifted-bilinear approach stays a proper bilinear. Two are equivalent ONLY at rest pose (where every warp is affine). Quantified results: PARAM mean 6.66 → 2.45 px (63% reduction); Breath_full 16.76 → 5.45 px (67%); body-chain fixtures roughly halved. AngleZ_pos30/neg30 still at 17.73 px — that's Phase 2b territory (rotation FD Jacobian Setup), still blocked on rotation-matrix-structure refactor.
- **2026-05-02** — Phase 3 lift diagnostics shipped: `chainEvalLift` log source (one-shot per rigSpec identity, lists every warp's lifted canvas-px bbox at Init Rig / cmo3 import / project load) + structured `warn` on cycle detection. LogsEditor now tints whole rows for warn (amber) + error (red, bold). Caught a long-standing footgun along the way: `.gitignore`'s unanchored `logs` rule was matching `src/v3/editors/logs/` — the entire LogsEditor.jsx had never been committed. Fixed by anchoring to `/logs`.
- **2026-05-02** — **Phase 1 kernel bug found via Phase 3 lift diagnostics → fixed (BUG-014).** User-reported "legwear stretched 2.5× canvas height below canvas after Init Rig + Body Angle slider unresponsive" on a man-character PSD whose legwear mesh extends just past canvas. The lift summary showed `RigWarp_legwear: y=[1170, 4163]` — bottom 2371 px below canvas, while `BodyXWarp` (its parent in the chain) covers canvas-y=229..1561. Pure linear projection canvas→BodyZ→BodyY→Breath→BX of legwear's bottom corner (canvas-y=3300) gives 3306, not 4163: **+860 px (26%) amplification poisoning the lift composition**. Re-decompiled `WarpDeformer_TransformTarget @ 0x7fff2b24cc40` via `mcp__ida-pro-mcp__decompile`. The bottom-band branch (`v29 >= 1.0`, ~`0x7fff2b24d784`) places real grid bottom row at virtual-cell **TL/TR** (boundary continuity at v=1) and extrapolated v=3 values at virtual-cell **BL/BR** (far below). v3's port had the assignments inverted — extrap on TL/TR, real on BL/BR — without the compensating top↔bottom swap that the top-band branch applies (lines 297-305). With `dv = (v - 1) / 2`, v_in=1 (boundary) → dv=0 reads TL/TR: Cubism reads real grid (continuous), v3 was reading extrap-at-v=3 (massively discontinuous). Phase 3's lifted-grid composition cascaded the per-step error multiplicatively through the chain. **Fix: byte-faithful port of IDA layout for the bottom-band branch only — TL/TR=real, BL/BR=extrap.** Verified all other 7 OOB branches (top, left, right, 4 corners, far-field) byte-match Cubism: top band has its own correct swap, left/right have natural asymmetric layouts that match, corner zones use a coordinate-sign-driven min/max mapping that derives the correct virtual cell. Tests stay green (breathFidelity 66, eyeClosureApply 35, chainDiagnose 38, etc.) — bug specifically affected the bottom-band region which the existing tests don't directly exercise. Follow-up: extend oracle harness with synthetic OOB-vertex tests covering each band's boundary so future kernel changes catch band-layout regressions before user-reported symptoms.
