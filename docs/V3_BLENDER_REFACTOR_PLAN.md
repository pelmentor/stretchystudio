...
| Live2D round-trip .cmo3 import | ⏳ partial — inspect-only | sweep #8 | `io/live2d/caffUnpacker.js` reverses the CAFF container (header + obfuscation key + file table + per-file XOR + ZIP / data-descriptor variants), so the embedded `main.xml` + PNG textures can be recovered. `io/live2d/cmo3Inspect.js` does a focused regex scan over `main.xml` for canvas / parameters / part-count / group-count / textures. New `Cmo3InspectModal` (driven by `cmo3InspectStore` + `file.inspectCmo3` operator) lets users drop a `.cmo3` and see what's inside without opening Cubism Editor. Verified against project-root `shelby.cmo3` — 137 parts, 34 groups, 31 parameters, canvas 1792×1792, all parameter IDs / ranges / defaults round-trip cleanly. **Still pending:** full project ingest (vertex / triangle / UV arrays, deformer chains, keyform grids, variants, masks, physics) — needs an XStream-style shared-pool resolver, multi-sweep follow-on. |
