...
| Live2D round-trip .cmo3 import | ⏳ partial — inspect + scene extraction | sweeps #8 + #9 | **Sweep #8:** `io/live2d/caffUnpacker.js` reverses the CAFF container (header + obfuscation key + file table + per-file XOR + ZIP / data-descriptor variants); `io/live2d/cmo3Inspect.js` does a regex scan over `main.xml` for header counts; new `Cmo3InspectModal` driven by `cmo3InspectStore` + `file.inspectCmo3` operator. **Sweep #9:** `io/live2d/cmo3XmlParser.js` is a hand-rolled XStream-style XML parser (~280 LOC; tag tree + xs.id pool + xs.ref helpers); `io/live2d/cmo3PartExtract.js` walks the tree and produces structured `ExtractedPart[]` (vertices in canvas px, triangle indices, UVs, texture xs.ref, parent guid xs.ref, clip mask refs, deformer guid xs.ref, draw order, visibility / lock flags) + `ExtractedGroup[]` (own guidRef so parts can join to groups, parent guidRef, name) + `ExtractedTexture[]` (resolved through GTexture2D → CImageResource → `file path="imageFileBuf_N.png"`). Inspector modal now shows the parts table + groups + texture file mapping. Verified against `shelby.cmo3`: 20 parts (irides-l/r, eyebrow-l/r, eyewhite-l/r, eyelash-l/r, …), 11 groups (Root Part → root → torso → neck → head/eyes/leftArm/rightArm), 20 textures with correct PNG path mapping. **Still pending:** full project ingest path (synth `project.nodes` / `project.textures` / `project.parameters` from the extracted scene), deformer chain decode (CWarpDeformerSource + CRotationDeformerSource + their parent links), keyform grids per parameter combination, variants, masks, physics rules, bone-baked angles. |
