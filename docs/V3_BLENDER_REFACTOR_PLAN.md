...
| Live2D round-trip .cmo3 import | ⏳ partial — load + structural deformer decode | sweeps #8–#11 | **#8:** `io/live2d/caffUnpacker.js` reverses the CAFF container; `io/live2d/cmo3Inspect.js` regex scan for header counts; `Cmo3InspectModal` + `cmo3InspectStore` + `file.inspectCmo3` operator. **#9:** `io/live2d/cmo3XmlParser.js` hand-rolled XStream-style XML parser; `io/live2d/cmo3PartExtract.js` produces typed `ExtractedPart[]` / `ExtractedGroup[]` / `ExtractedTexture[]`; inspector modal shows parts table + group hierarchy + texture file mapping. **#10:** `io/live2d/cmo3Import.js` (`importCmo3` → `{project, warnings, stats}`) synthesises a full SS project from the extracted scene: groups + parts as nodes (parent links stitched through CPartGuid intermediary), parts get `mesh = {vertices:[{x,y,restX,restY}], uvs:Float32Array, triangles:[[i,j,k],…], edgeIndices:Set}` in canvas pixel space, each part's GTexture2D xs.ref resolves to a `imageFileBuf_N.png` blob URL bound by `node.id === texture.id`, parameters carry through with their min/max/default, ParamOpacity gets `role='opacity'`. Inspector modal now exposes an **Import as new project** button. Verified end-to-end against `shelby.cmo3`: 31 nodes (11 groups + 20 parts, full Root Part → root → torso → neck → head/eyes/limbs hierarchy decoded), 20 textures bound, 31 parameters with correct roles. **Still pending:** deformer chain decode (CWarpDeformerSource + CRotationDeformerSource + parent links), keyform grids, parameter bindings, variants, masks, physics rules, bone-baked angles — the import currently produces a static reference scene, the rig path is the next set of sweeps. |
