...
**What it is.** `editorMode: 'staging' | 'animation'` is a stored slot in `editorStore`. After PP2-009 it is **always** equal to `(activeWorkspace === 'animation' ? 'animation' : 'staging')` — the only writer is `setWorkspace`. Any other write would desync it from the workspace, but every existing call site already pairs with a workspace switch.
...
**Plan.**
1. **Replace reads.** ~49 references across 10 files. Each `useEditorStore(s => s.editorMode)` becomes `useUIV3Store(s => s.activeWorkspace === 'animation' ? 'animation' : 'staging')` — or, more cleanly, a derived selector exported from `uiV3Store`:
   ```js
   export const selectEditorMode = (s) => s.activeWorkspace === 'animation' ? 'animation' : 'staging';
   ```
2. **Move rest-pose capture out of EditorModeService.** Today `setEditorMode('animation')` calls `captureRestPose` on staging→animation. Move that side-effect into `setWorkspace` directly (or into a workspace-transition hook).
3. **Remove `editorStore.editorMode` + `setEditorMode` from editorStore.** Delete `EditorModeService` once no readers remain — its only function was the rest-pose capture, which lives at the workspace transition now.
4. **Remove the redundant `serviceSetEditorMode('animation')` calls** in `IdleMotionDialog.jsx:159` and `AnimationsEditor.jsx:119` (they fire after `setWorkspace('animation')`, which now does the same thing).
...
**Why it might be a crutch.** Blender's pattern: explicit `I` press to insert a keyframe, full stop. There's a per-property "Auto Keyframe" toggle in the timeline header for users who want Spine-style behaviour, but it's NOT on by default and it's a timeline-local control, not a global flag.
...
**Why it's a crutch.** Pure naming hazard. Multiple bugs in the past have come from grepping `editMode` and getting `editorMode` matches (or vice versa). When BFA-001 lands and `editorMode` is removed, this collapses naturally — but until then, any new contributor needs to stay alert.
