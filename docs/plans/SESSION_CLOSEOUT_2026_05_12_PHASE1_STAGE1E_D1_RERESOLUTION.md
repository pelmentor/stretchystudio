...
This sub-session (D-1 RE-RESOLUTION) is decoupled polish that closes
Rule №2 doc baggage on the Properties animation panel narrative;
doesn't move the Phase 1 ship gate.
...
only with no PSD), create one Action via ActionsEditor, add
keyframes, bind to `__scene__`, export, open in Cubism Viewer 5.0 +
Cubism Editor 5.0 → Animation workspace.
