...
memory `feedback_test_character_is_shelby.md`). This sub-session (D-1
RE-RESOLUTION) is decoupled polish that closes Rule №2 doc baggage on
the Properties animation panel narrative; doesn't move the Phase 1
ship gate.
...
fly was anime-only. Hiyori is reference-only with no PSD source
(plan §11 line 1627).
